...
new_columns.extend(['from-' + col for col in ['active_duration', 'avg_gas_price', 'unique_in_degree', 'avg_amount_incoming', 'out_degree', 'avg_amount_outgoing', 'mean_time_interval']])
new_columns.extend(['to-' + col for col in ['active_duration', 'avg_gas_price', 'unique_in_degree', 'avg_amount_incoming', 'out_degree', 'avg_amount_outgoing', 'mean_time_interval']])
...
for _, row in transactions_df.iterrows():
    ...
    label_value = int(from_addr_info['label'].values[0] or to_addr_info['label'].values[0])

    avg_values = list()
    avg_values.append(hash)
    avg_values.append(from_address)
    avg_values.append(to_address)
    avg_values.extend(list(from_addr_info[['active_duration', 'avg_gas_price', 'unique_in_degree', 'avg_amount_incoming', 'out_degree', 'avg_amount_outgoing', 'mean_time_interval']].values[0]))
    avg_values.extend(list(to_addr_info[['active_duration', 'avg_gas_price', 'unique_in_degree', 'avg_amount_incoming', 'out_degree', 'avg_amount_outgoing', 'mean_time_interval']].values[0]))
